...
%In addition, \citet{Delong1990noise} and \citet{shleifer1997limits} show that constrained horizons can cause noise-trading to limit the ability of informed traders to take advantage of profitable opportunities for fear of further divergence from fundamental value before realignment is achieved.
...
The \citet[pp. 14-15]{Delong1990noise} model is summarised by Equation \ref{eq:NP2}. There are two parameters that can be used to measure speculation:  the price misperception $(\rho)$ and the weight of noise-traders in the market $(\mu)$.  If speculation is uninformed and either of these parameters reach an extreme, the model indicates that prices should subsequently reverse.\footnote{See \citet{Hayward2013} for full details of a Monte-Carlo simulations of this model when the speculative misperception $(\rho)$ and the weight of speculation $(\mu)$ parameters are adjusted.} Therefore, if misperception or weight of speculation can be measured, this assertion can be tested. 
...
That sentiment drives price is evidence that speculators have influence on the market.   This is consistent with the evidence of previous research from the likes of \citet{Evans2002Order}, \citet{FuturesSanders} and \citet{FuturesWang}.  What is more interesting is whether knowledge about the extreme reading at time $t = 0$ provides any information about future returns.  If this speculation is noise, informed traders should take advantage of the space that has been created between the price and the fundamental value and a reversal should be seen.  Whether this is the case is recorded in columns 5, 8, 11 and 14 of Table \ref{tabref:RR1}. A reversal is indicated by a significant negative reading for highs or positive reading for lows.   Looking, for example, at the four-day window in column 8, it is clear that the extreme risk reversal skew does not provide any information about future returns.  Of the eighteen cases using the $95^{th}$ percentile to identify an extreme, ten show a continuation rather than a reversal that is not significantly different from zero; one shows a significant continuation; five show reversals that are not significant; only two show significant reversal (EURCHF in each case).  For the more extreme event when the $99^{th}$ percentile is used to measure the extreme, there is even more ambiguity.  Just over half the cases with the four-day window show a reversal (8 of 18), but only 2 of these are statistically significant. Exchange rate prices are as likely to go up as down after an extreme and there does not appear to be any information about the future from knowledge of these extremes.  
\begin{figure}
\graphicspath{{../Figures/}}
\centering
\includegraphics[scale=0.8]{RRCum16}
...
It is also possible to look at the distribution exchange rate returns after these extremes and to compare them to the distribution of the whole sample.  If speculation is informed, we expect to see a distribution of return that is largely symmetrical around zero and very similar to the distribution for the whole sample. The results are presented in Table \ref{tabref:D}.  They show that the distribution of returns is very similar but, but much broader than that of the whole distribution.  Conventionally, this would suggest that there is greater risk.  It may be that the increase in information adds to the level of uncertainty in the market. 
% Add the table with the descriptive statistics for returns that follow an extreme compared to the whole sample. 
\begin{table}[h]
\begin{threeparttable}
\begin{tabular}{p{3cm} rrllll}
& \multicolumn{4}{c}{Extreme Distribution}&\multicolumn{2}{c}{Regular Distribution}\\
& \multicolumn{2}{c}{High}&\multicolumn{2}{c}{Low}&
\multicolumn{2}{l}{}\\
 & Mean & Std & Mean & Std & Mean & Std \\
\hline
AUD & -0.0174 & -0.0017 &  &  & 0.7241 & 0.0106 \\ 
  EURGBP & -0.0187 & 0.0010 &  &  & 0.5542 & 0.0090 \\ 
  EURSEK & 0.0011 & 0.0014 &  &  & 0.4384 & 0.0061 \\ 
  GBP & 0.0025 & 0.0003 &  &  & 0.4892 & 0.0093 \\ 
  CHF & 0.0120 & -0.0011 &  &  & 0.6819 & 0.0135 \\ 
  EURCHF & -0.0039 & 0.0002 &  &  & 0.2769 & 0.0054 \\ 
  EURJPY & -0.0072 & -0.0016 &  &  & 0.8416 & 0.0154 \\ 
  EUR & -0.0172 & -0.0009 &  &  & 0.6338 & 0.0094 \\ 
  CAD & 0.0084 & -0.0006 &  &  & 0.3632 & 0.0052 \\ 
  JPY & 0.0076 & 0.0009 &  &  & 0.8170 & 0.0139 \\ 
...
Figures \ref{fig:ES3} and \ref{fig:ES4} indicate that these results are not a function of the event window that has been chosen.  Looking at the $95^{th}$ percentile confidence intervals (once again constructed by bootstrap re-sampling) indicates that in all the cases, there is a large amount of uncertainty about the future direction well after the extreme reading of speculative activity.  It does not appear that informed traders are constrained in the short-run from driving prices back to equilibrium.  It appears that the informed traders are driving prices to a new level. 

\section{Speculation and foreign exchange returns}
The nature speculative activity in financial markets is central to a number of contemporary policy questions.  Amongst these are the design and regulation of financial markets, the response to the surge in commodity prices that took place between the period 2002 and 2007 and the limits that should be put on the activities of financial institutions. This paper provides some evidence to assist in policy making in these areas.   

The debate about a tax on financial transactions, initiated by \citet{TobinTax} and now called the \emph{the Tobin tax}, has been intended to reduce speculation. This is based on the belief that if speculation is uninformed, that it drive prices away from their fundamental value and, as a consequence, increases the volatility of financial asset prices. The argument for a tax on financial transactions can be presented as a tax on an externalities with the level of volatility of financial markets being the public cost.  
...
Regulatory changes in the US government bond market give some hints at the way that reduce speculation can affect the smooth functioning of markets. Increased capital requirements and the so-called \emph{Volcker Rule} that would prevent investment banks from trading on their own account have increased the cost of trading US government bonds and reduced the ability of some key players to participate in the market. Though bid-ask spreads have remained stable, the New York Federal Reserve reports that \emph{market-depth} and the \emph{price impact} of large trades has deteriorated in the last two years.  This is regarded as one of the reason that volatility has increased at times of tension and as liquidty has evaporated and large price changes have been recorded. 
